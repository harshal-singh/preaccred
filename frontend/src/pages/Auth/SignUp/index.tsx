...
const EmailAndPasswordSignUpForm = () => {
  ...
  const handleConfirmPasswordChange = useCallback(
    (ev: ChangeEvent<HTMLInputElement>, data: InputOnChangeData) => {
      if (data.value !== getValues('password') && data.value !== '') {
      ...
      setValue('confirmPassword', data.value);
    },
      ...
      <Controller
        ...
        render={({
          ...
          <Field
            ...
            label="Full Name"
            className="h-fit"
            validationState={errors.fullName && 'error'}
      ...
      <Controller
        ...
        render={({
          ...
          <Field
            ...
            className="h-fit"
            validationState={errors.confirmPassword && 'error'}
  ...
  return (
    <div className="w-full min-h-screen grid place-items-center bg-tint40">
      <div className="flex flex-col gap-6 border rounded-xl p-6 w-80 bg-white">
        ...
        <Divider>OR</Divider>
        <GoogleSignUpButton />
        <Link to="/">Already signed up? please sign in.</Link>
      </div>
    </div>
  );
};
